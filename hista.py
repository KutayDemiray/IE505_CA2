    ...
    for k in range(max_iter):
        ...
        if line_search:
            ...
            while True:
                ...
                rhs = np.sum(huber_loss(A @ x - b, gamma)) - eta_tmp * np.dot(grad, G_eta) + eta_tmp / 2 * np.linalg.norm(G_eta)**2
                if lhs <= rhs:
                    break
                eta_tmp *= beta
            eta = eta_tmp
...
def FastHISTA(A, b, lam, gamma, line_search=False, init_t_with_L=False, max_iter=100000, tol=1e-6, beta=0.9):
    x = np.zeros(A.shape[1])
    ...
    for k in range(max_iter):
        ...
        if line_search:
            ...
            while True:
                ...
                lhs = np.sum(huber_loss(A @ x_new - b, gamma))
                rhs = np.sum(huber_loss(A @ y - b, gamma)) - eta_tmp * np.dot(grad, G_eta) + eta_tmp / 2 * np.linalg.norm(G_eta)**2
                if lhs <= rhs:
                    break
...
def ProxNewton(A, b, lam, gamma, line_search=False, max_iter=100000, tol=1e-6, alpha=0.1, beta=0.5):
    x = np.zeros(A.shape[1])
    ...
    for k in range(max_iter):
        ...
        W = np.diag([1.0 / gamma if np.abs(residual[i]) <= gamma else 0.0 for i in range(len(residual))])
    ...
    for k in range(max_iter):
        ...
        if line_search:
            ...
            while True:
                ...
                lhs = l2_new
                rhs = l2_current - eta_tmp * np.dot(grad, G_eta) + (eta_tmp / 2) * np.linalg.norm(G_eta)**2
